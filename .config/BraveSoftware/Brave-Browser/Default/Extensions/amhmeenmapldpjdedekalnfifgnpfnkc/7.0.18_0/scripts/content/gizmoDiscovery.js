/* global getGizmoDiscovery, dropdown, isDarkMode, addDropdownEventListener, showConfirmDialog, openUpgradeModal, deleteGizmo, toast, debounce, gizmoSortByList, getGizmosBootstrap, updateGizmoSidebar, getGizmoAbout, showGizmoAboutDialog, loadingSpinner, translate, closeMenus */
...
function renderGizmoDiscoveryPage(defaultCategory = 'all') {
  ...
  chrome.runtime.sendMessage({
    ...
    chrome.storage.sync.get(['openai_id']).then((syncRes) => {
      ...
      gizmoHeaderWrapper.classList = `${isDarkMode() ? 'bg-black' : 'bg-token-main-surface-primary'} pb-1 pt-4`;
      ...
      gizmoGrid.style = 'display:flex;flex-direction:row;flex-wrap:wrap;justify-content:start;align-items:stretch;height:100%; overflow-y: auto;padding-bottom: 120px;';
...
function gizmoManagerModalActions() {
  ...
  createGPTButton.innerText = translate('Create a GPT');
...
function renderGizmoGrid(gizmos, gizmoType, currentUserId, hasSubscription = false, newPage = true) {
  ...
  gizmos?.forEach((gizmo) => {
    ...
    gizmoCardWrapper.classList = 'flex flex-col w-full justify-start items-start gap-2 pr-3 pb-3 h-max';
    ...
    gizmoCardWrapper.innerHTML = `
      <div id="gizmo-card-${gizmo.id}" class="relative flex flex-col w-full h-full justify-start items-start gap-2 p-4 cursor-pointer bg-token-main-surface-secondary hover:bg-token-main-surface-tertiary hover:shadow-xl rounded-xl">
        ...
        <div class="text-sm text-token-text-secondary" style="min-height:80px; white-space: break-spaces; overflow-wrap: break-word;display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden;word-break:break-word;">${gizmo.display.description || '...'}</div>
