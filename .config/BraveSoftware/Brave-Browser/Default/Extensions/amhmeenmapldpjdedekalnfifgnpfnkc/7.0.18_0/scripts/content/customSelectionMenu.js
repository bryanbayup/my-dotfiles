/* global isDescendant, TurndownService, closeMenus */
function customSelectionMenuShow(x, y, _selection) {
  const menu = document.querySelector('#custom-selection-menu');
  if (!menu) return;
...
function customSelectionMenuQuote() {
  ...
  const existingReplyToPreviewElement = document.querySelector('#reply-to-preview-wrapper');
  if (existingReplyToPreviewElement) existingReplyToPreviewElement.remove();

  const taggedGizmoElement = document.querySelector('#tagged-gizmo-wrapper');
  if (taggedGizmoElement) {
    taggedGizmoElement.classList.remove('rounded-t-[20px]');
  }
  const imageEditSelectionPreviewElement = document.querySelector('#image-edit-selection-preview');
  if (imageEditSelectionPreviewElement) {
    imageEditSelectionPreviewElement.remove();
  }
  const replyToPreviewElement = `<div id="reply-to-preview-wrapper" class="relative flex items-start gap-3 text-sm py-2.5 pl-3 pr-1.5 bg-token-main-surface-primary text-token-text-secondary rounded-t-[20px] ${taggedGizmoElement ? '' : 'rounded-b-lg'} border border-token-border-light">
  ...
  textAreaElement.parentElement.insertAdjacentHTML('afterbegin', replyToPreviewElement);
  ...
  closeButton.addEventListener('click', (e) => {
    ...
    if (curTaggedGizmoElement) {
      curTaggedGizmoElement.classList.add('rounded-t-[20px]');
    }
  });
  // clear selection
...
function addCustomSelectionMenuEventListener() {
  ...
  const quoteReplyButton = document.querySelector('#quote-reply');
