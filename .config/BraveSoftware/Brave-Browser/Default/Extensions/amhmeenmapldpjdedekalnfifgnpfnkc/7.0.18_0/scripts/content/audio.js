/* global isFirefox, isOpera, toast, playingAudios:true, setSelectionAtEnd */
...
function addAudioEventListener() {
  // get audio with class="fixed bottom-0 left-0 hidden h-0 w-0
  const audio = document.querySelector('audio.fixed.bottom-0.left-0.hidden.h-0.w-0');
  if (!audio) {
    return;
  }
  audio.addEventListener('ended', () => {
    autoSpeackQueue.shift()?.click();
    if (autoSpeackQueue.length > 0) {
